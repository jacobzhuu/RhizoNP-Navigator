# Architecture Overview

RhizoNP Navigator is an evidence-grounded AI-for-Science pipeline — **not** a single RAG chatbot or autonomous agent.

```text
Own omics CSV / demo fixtures
        │
        ▼
Query construction + literature retrieval bridge (Phase 5.1/5.2)
        │
        ▼
Taxonomy normalization + evidence grading (Phase 3)
        │
        ▼
Natural product candidate linking (Phase 4)
        │
        ├──────────────┐
        ▼              ▼
Literature retrieval   Structured NP fixtures
(Phase 2)              (Phase 4)
        │              │
        └──────┬───────┘
               ▼
Evidence-grounded writer (Phase 6)
               │
               ▼
Evaluation + demo package (Phase 7/8)
```

## Core packages

| Package | Responsibility |
|---|---|
| `rhizonp.domain` | SQLAlchemy schema |
| `rhizonp.literature` | Chunking, retrieval, provenance |
| `rhizonp.taxonomy` | Bounded NCBI cache + fixture normalization, rank-aware distance, evidence tier policy |
| `rhizonp.linking` | Natural product candidate matrix |
| `rhizonp.omics` | Own-data CSV ingestion, literature query bridge, pipeline |
| `rhizonp.writer` | Deterministic grounded answer synthesis |
| `rhizonp.evaluation` | Retrieval and end-to-end metrics |
| `rhizonp.demo` | Offline smoke/demo runner |
| `rhizonp.api` | FastAPI integration layer |

## Artifact layout

| Location | Tracked in git | Purpose |
|---|---|---|
| `data/fixtures/` | Yes | Public-safe deterministic demo inputs |
| `data/eval/*.json` (case definitions) | Yes | Benchmark and eval case specs |
| `data/output/` | No (gitignored) | Regenerated by `make smoke` / `make demo` |
| `data/eval/reports/latest/` | No (gitignored) | Regenerated by `make eval-end-to-end` and `make validate-real-pubmed-bridge` |

## Corpus provenance (Phase 5.2)

Literature hits carry explicit corpus identity:

| `corpus_type` | Meaning |
|---|---|
| `REAL_BOUNDED_PUBMED` | Records from bounded NCBI E-utilities fetch (`rhizonp_domain_v1`) |
| `FIXTURE_TEST_ONLY` | Synthetic/offline demo literature (e.g. Phase 2 fixture) |
| `SYNTHETIC` | Generated test data |
| `UNKNOWN` | Unclassified stored paper |

Own-data bridge retrieval states (`DISABLED`, `RETRIEVED`, `NO_RESULTS`, `RETRIEVAL_UNAVAILABLE`, `FIXTURE_TEST_ONLY`) are unchanged; real corpus hits add `corpus_type` and `corpus_id` rather than implying PubMed-wide retrieval.

## Taxonomy authority (Phase 3)

| Source | Meaning |
|---|---|
| `ncbi_bounded` | Match from offline bounded NCBI cache (`data/snapshots/taxonomy/ncbi_bounded_v1/`) |
| `fixture` | Local alias fixture (`data/fixtures/taxonomy_mapping.json`) |
| `auto` | Prefer `ncbi_bounded` when cache file and label match exist; explicit fixture fallback otherwise |

Resolution metadata (`requested_source`, `resolved_source`, `fallback_reason`, `cache_id`) is attached to normalized taxa and grading provenance. Live NCBI network lookup is opt-in (`ncbi_live`), not the default.

## Design principles

- Taxonomy-aware evidence grading prevents genus-level 16S observations from becoming strain-level production claims.
- Every answer binds claims to evidence references or abstains (`INSUFFICIENT_EVIDENCE`, `CONFLICTING_EVIDENCE`).
- External database integrations (NPAtlas, MIBiG, etc.) are **not** part of the current MVP.
- Metrics are reported honestly from deterministic replay cases, not fabricated benchmark gains.
- Phase 2 real PubMed human relevance evaluation remains pending; no PubMed-wide performance claims.
